...
(defcustom ac-use-fuzzy (and (locate-library "fuzzy") nil)
...
(defcustom ac-quick-help-delay 3.5
...
(defvar ac-completing-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-TAB") 'auto-complete)

;;    (define-key map "\M-n" 'ac-next)
;;    (define-key map "\M-p" 'ac-previous)
    (define-key map [C-down] 'ac-next)
    (define-key map [C-up] 'ac-previous)
